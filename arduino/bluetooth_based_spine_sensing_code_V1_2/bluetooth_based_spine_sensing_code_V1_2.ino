...
void loop(){
  ...
  for( i=0 ; i <= 7 ; i++){
    ...
    //btSerial.print(" : ");
    //btSerial.print(round(-0.1025*(analogRead(i) - 1024/2)));
    btSerial.print(",");
  }
  //btSerial.print("]");
  ...
  if(DEBUG){
    ...
    for( i=0 ; i <= 7 ; i++){
      ...
      //Serial.print(round(-0.1025*(analogRead(i) - 1024/2)));
      Serial.print(",");
    }
    //Serial.print("]");
  }
  ...
  for (analogMuxCount = 0; analogMuxCount <=8; analogMuxCount++){ // analogmux read side sensor start - JD
    ...
    delay(5); // give ADC time to settle
  }     // analogmux read side sensor end - JD


  sendSerial("\n");

  delay(200); // give ADC time to settle
